...
async function onMessage(m) {
  ...
  if(m.id!==undefined&&m.method){
    ...
    requests.set(String(m.id),m);refreshRequest(t);return;
  }
  ...
  switch(m.method){
    ...
    case 'item/agentMessage/delta': t.messages[p.itemId]=(t.messages[p.itemId]||'')+p.delta;t.activity='正在回复';break;
    case 'item/started': {const i=p.item;t.status='working';t.activity=({commandExecution:'执行命令',fileChange:'修改文件',webSearch:'查阅资料',reasoning:'分析中',mcpToolCall:'使用工具',collabAgentToolCall:'协调伙伴',contextCompaction:'整理上下文'})[i.type]||'正在工作';if(i.type==='commandExecution')log(t,'运行命令',i.command);if(['collabAgentToolCall','subAgentActivity','userMessage'].includes(i.type))resultItem(t,i);break;}
    case 'item/completed': resultItem(t,p.item);break;
...
async function startTaskUnlocked(body){
  ...
  const t={id,profileId:profile.id,name:profile.name||'伙伴',parentId:null,workspace,prompt:body.prompt,status:'starting',activity:'准备任务',result:'',messages:{},logs:[],files:[],created:Date.now(),updated:Date.now(),snapshot:structuredClone(profile),roster:structuredClone(roster),turnId:null,request:null};tasks.set(id,t);changed(t);
  try {const turn=await rpc('turn/start',{threadId:id,input:input(body.prompt,chosen)});t.turnId=turn.turn.id;if(t.status==='starting')t.status='running';changed(t);}catch(e){t.status='failed';t.error=e.message;changed(t);throw e;}
  return {task:t};
}
...
async function messageTask(body){const t=tasks.get(body.id);if(!t)throw Error('任务不存在');if(!body.text?.trim())throw Error('请输入指令');await connect();
  ...
  let routed=false;
  if(active.has(t.status)&&t.turnId){try{await rpc('turn/steer',{threadId:t.id,expectedTurnId:t.turnId,input:input(body.text)});routed=true;}catch(e){if(t.parentId)return relayMessage(t,body.text);throw e;}}
  if(!routed){const previous=t.status;t.status='starting';try{const r=await rpc('turn/start',{threadId:t.id,input:input(body.text)});t.turnId=r.turn.id;if(!['completed','failed','interrupted'].includes(t.status))t.status='running';}catch(e){t.status=previous;changed(t);if(t.parentId)return relayMessage(t,body.text);throw e;}}
...
async function answerRequest(body){const req=requests.get(String(body.id));if(!req)throw Error('请求已结束');const method=req.method;let result;
  ...
  write({id:req.id,result});requests.delete(String(body.id));const t=tasks.get(req.params.threadId);if(t){refreshRequest(t);log(t,'用户回应',approval?.label||(body.accept?'已允许':body.answers?'已提交回答':body.answer||'已拒绝'));}return{ok:true};}
function publicState(){return {revision,connected,account,error:lastError,tasks:[...tasks.values()].map(t=>({...t,messages:Object.values(t.messages).join('\n\n').slice(-48000),logs:t.logs.slice(-60)}))};}
